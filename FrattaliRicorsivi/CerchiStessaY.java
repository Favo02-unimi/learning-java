...
public class CerchiStessaY extends Canvas {

    //solita roba per creare la finestra
    public static void main(String[] args) {
        JFrame frame = new JFrame("Cerchi, troppi cerchi sulla stessa y");
        frame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
        frame.setBounds(0,0, 450,450);
        CerchiStessaY disegno = new CerchiStessaY();
    ...
    public void drawCircle(Graphics g, int x, int y, int raggio) {
        ...
        if (raggio > 2) {
            //cambia la posizione (solo x) e il raggio
            drawCircle(g, x + raggio/2, y, raggio/2);
            drawCircle(g, x - raggio/2, y, raggio/2);
        }
    }
